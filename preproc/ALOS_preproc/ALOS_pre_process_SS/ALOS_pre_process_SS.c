 ...
 * 07/15/08  added a command-line option to force earth radius DTS * 10/02/08
 * added capability to pre_process ScanSAR WB1 data DTS               *
 * *****************************************************************************/

#include "image_sio.h"
#include "lib_functions.h"

char *USAGE = "\n\nUsage: ALOS_pre_process_SS imagefile LEDfile [-near near_range] "
              "[-radius RE] [-swath swath#] [-burst_skip] [-num_burst] [-swap] [-V] "
              "[-debug] [-quiet] \n"
              ...
              "-burst_skip            number of burst to skip before starting output "
              "(1559 lines/burst) \n"
              ...
              "Example:\n"
              "ALOS_pre_process_SS IMG-HH-ALPSRS049842950-W1.0__D "
              "LED-ALPSRS049842950-W1.0__D -near 847916 -radius 6371668.872945 "
              "-burst_skip 5 -num_burst 36 \n\n"
...
int main(int argc, char **argv) {
	...
	if ((imagefile = fopen(argv[1], "rb")) == NULL)
		die("couldn't open Level 1.0 IMG file \n", argv[1]);
	...
	if (burst_skip * 1559 > 100000)
		fprintf(stderr, " warning skip could exceed the length of the file \n");
