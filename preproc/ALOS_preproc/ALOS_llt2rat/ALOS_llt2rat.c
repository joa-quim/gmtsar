...
char *USAGE = " \n Usage: "
              "ALOS_llt2rat master.PRM prec [-bo[s|d]] < inputfile > outputfile  \n\n"
              "             master.PRM   -  parameter file for master image and points "
              "to LED orbit file \n"
              "             precise      -  (0) standard processing, (1) - polynomial "
              "refinenent for backprojection (slower) \n"
...
int main(int argc, char **argv) {
	...
	double ts, rng0, thet, relp, telp, dopc;
	double xp[3];
	double xt[3];
	double rp[3];
	...
	double dt, xs, ys, zs;
	double time[10], rng[10], d[3]; /* arrays used for polynomial refinement of min range */
	...
	*/
	...
	while (scanf(" %lf %lf %lf ", &rln, &rlt, &rht) == 3) {
		...
		/* compute the range and azimuth in pixel space and correct for ALOS-1 and
		   ALOS-2 Note that ALOS-1 uses the range and azimuth corrections from the
		   non-zero Doppler.
		   Note that ALOS-2 is corrected to zero Doppler so these corrections are
		   not needed.*/
		...
		if (prm.SC_clock_start / 1000. > 2014.) {
			...
			xt[0] = (xt[0] - prm.near_range) / dr - (prm.rshift + prm.sub_int_r) + prm.chirp_ext;
			xt[1] = prm.prf * (xt[1] - t1) - (prm.ashift + prm.sub_int_a) + 4;

			/* compute the azimuth and range correction for ALOS-1 if the Doppler is
			 * not zero */

			// printf(" year fd1 fdd1 %f %f %f
			// \n",prm.SC_clock_start/1000.,prm.fd1,prm.fdd1);
			if (prm.fd1 != 0.) {
				...
				daa = prm.prf * daa;
				/*printf(" vel lambda dopc prf %f %f %f %f
				\n",prm.vel,prm.lambda,dopc,prm.prf);
				printf(" rng, rdd, daa, drr xt[1]  %f %f %f %f %f \n",rng, rdd, daa,
				drr, xt[1]); */
				xt[0] = xt[0] + drr;
				xt[1] = xt[1] + daa;
			}
		...
		if (precise == 1)
			xt[1] = xt[1] - .5; /* this correction is needed for the precise to match
			                       the standard on average */
