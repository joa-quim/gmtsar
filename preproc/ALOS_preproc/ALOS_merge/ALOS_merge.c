 ...
 *******************************************************************************/
/********************************************************************************
 * Creator:  Rob Mellors * (San Diego State University) * Date   :  10/03/2007 *
 ********************************************************************************/
/********************************************************************************
...
char *USAGE = "Usage: \n"
              ...
              "example: ALOS_merge IMG-HH-ALPSRP057130830-H1.0__A.PRM "
              "IMG-HH-ALPSRP057130840-H1.0__A.PRM -outout foo -V\n";
...
int main(int argc, char **argv) {
	...
	strcpy(tmp, "merge");
	if (argc > narg)
		parse_ALOS_merge(argc, narg, argv, tmp, USAGE);
	assign_output(RawOutName, PRMOutName, tmp);

	/* open files */
	if ((PRMfile1 = fopen(argv[1], "r")) == NULL)
		die("can't open prfile", argv[1]);
	if ((PRMfile2 = fopen(argv[2], "r")) == NULL)
		die("can't open prfile", argv[2]);
	if ((PRMfile3 = fopen(PRMOutName, "w")) == NULL)
		die("can't open prfile", PRMOutName);
	...
	if (overlap < 0)
		die("frames do not overlap in time", "");
	if (verbose)
		fprintf(stderr, " frame 1 %lf %lf\n frame 2 %lf %lf overlap %lf\n", p1.SC_clock_start, p1.SC_clock_stop,
	...
	start_frame = -1;
	if (verbose)
		fprintf(stderr, " reading file 1 %s\n", p1.input_file);
	nlines1 = read_ALOS_raw_data(RAWfile1, RAWfile3, &p1, &start_frame);

	if (verbose)
		fprintf(stderr, " reading file 2 %s\n", p2.input_file);
	...
	if (verbose)
		fprintf(stderr, " nlines1 %ld nlines2 %ld nlines %ld \n", nlines1, nlines2, nlines);
...
int64_t  read_ALOS_raw_data(FILE *imagefile, FILE *outfile, struct PRM *prm, int *start_frame) {
	...
	if (debug)
		fprintf(stderr, ".... reading data (%d bytes)\n", line_prefix_size + line_data_size);
	...
	while ((fread((void *)&sdr, line_prefix_size, 1, imagefile)) == 1) {
		...
		if (n == 0) {
			first_frame = frame;
			if (verbose)
				fprintf(stderr, "first frame %d (start_frame %d)\n", first_frame, *start_frame);
		}
		...
		if (frame > *start_frame) {
			if (verbose)
				if (nlines == 0)
					fprintf(stderr, " first line written %ld frame %d \n", n, frame);
	...
	*start_frame = frame;
	...
	if (verbose)
		fprintf(stderr, "last frame %d (nlines written %ld)\n", *start_frame, nlines);
...
double get_clock(struct sardata_info sdr, double tbias) {
	...
	time = ((double)sdr.sensor_acquisition_year) * 1000 + (double)sdr.sensor_acquisition_DOY +
	       (double)sdr.sensor_acquisition_msecs_day / 1000.0 / 86400.0 + tbias / 86400.0;
