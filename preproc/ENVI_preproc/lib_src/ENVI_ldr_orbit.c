...
void ENVI_ldr_orbit(struct ALOS_ORB *orb, struct PRM *prm) {
	...
	calc_height_velocity_envi(orb, prm, t2, t2, &height_end, &re_end, &vg_end, &vtot, &rdot);
	calc_height_velocity_envi(orb, prm, t1, t2, &height, &re_c, &vg, &vtot, &rdot);
...
void calc_height_velocity_envi(struct ALOS_ORB *orb, struct PRM *prm, double t1, double t2, double *height, double *re2,
                               double *vg, double *vtot, double *rdot) {
	...
	if (verbose)
		fprintf(stderr, " ... calc_height_velocity_envi\n");
